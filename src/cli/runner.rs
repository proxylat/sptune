use crate::backend::{IoEvent, Network};
use crate::user_config::UserConfig;

use super::format::{Flag, Format, FormatType, JumpDirection, Type};

use anyhow::{anyhow, Result};
use rspotify::clients::BaseClient;
use rspotify::model::{context::CurrentPlaybackContext, Id, PlayableItem};
...
impl<'a> CliApp<'a> {
  ...
  // sptune pb --share-track (share the current playing song)
  ...
  // sptune pb --share-album (share the current album)
  ...
  pub async fn share_album_or_show(&mut self) -> Result<String> {
    ...
    }) = &app.current_playback_context
    {
      ...
      Err(anyhow!(
        "failed to generate a share url for the current song"
      ))
    }
  }

  // sptune ... -d ... (specify device to control)
      ...
      for (i, d) in dp.iter().enumerate() {
        if d.name == name {
          ...
          self
            .net
            .client_config
            .set_device_id(d.id.clone().unwrap_or_default())
  ...
  // sptune query ... --limit LIMIT (set max search limit)
  ...
  // sptune playback --transfer DEVICE
  pub async fn transfer_playback(&mut self, device: &str) -> Result<()> {
    ...
    if let Some(devices) = &self.net.app.lock().await.devices {
      for d in devices {
        if d.name == device {
          id.push_str(d.id.as_deref().unwrap_or_default());
          break;
        }
      }
    };
  ...
  pub async fn seek(&mut self, seconds_str: String) -> Result<()> {
    let seconds = match seconds_str.parse::<i64>() {
      Ok(s) => s.abs(),
      Err(_) => return Err(anyhow!("failed to convert seconds to i64")),
    };
    ...
    if position_to_seek > duration {
      ...
      self
        .net
        .handle_network_event(IoEvent::Seek(position_to_seek as u32))
        .await;
    }
  ...
  // sptune playback --like / --dislike / --shuffle / --repeat
  ...
  pub async fn get_status(&mut self, format: String) -> Result<String> {
    ...
    let mut hs = match playing_item {
      PlayableItem::Track(track) => {
        ...
        if let Some(p) = context.progress {
          hs.push(Format::Position((
            p.num_milliseconds() as u32,
    ...
    let offset = if random {
      ...
      if uri.contains("spotify:playlist:") {
        ...
        match self
          ...
          .spotify
          .playlist(
            rspotify::model::PlaylistId::from_id_or_uri(id).unwrap(),
            None,
            None,
          )
          .await
        {
          Ok(p) => {
            ...
            Some(crate::lcg::rand_idx(num as usize))
          }
          Err(e) => {
            self
              .net
