...
use super::{
  format::{Flag, JumpDirection, Type},
  CliApp,
};
...
pub async fn handle_matches<'a>(
  ...
  let devices_list = match &cli.net.app.lock().await.devices {
    Some(p) => p
      .iter()
      .map(|d| d.id.clone().unwrap_or_else(|| "".to_string()))
      .collect::<Vec<String>>(),
    None => Vec::new(),
  };
  ...
  let output = match cmd.as_str() {
    "playback" => {
      ...
      if let Some(vol) = matches.get_one::<String>("volume") {
        cli.volume(vol.clone()).await?;
      }
      if let Some(secs) = matches.get_one::<String>("seek") {
        cli.seek(secs.clone()).await?;
      }
    ...
    "list" => {
      ...
      if let Some(max) = matches.get_one::<u32>("limit") {
