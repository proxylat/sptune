...
use rspotify::{
  ...
  model::{
    ...
    playlist::SimplifiedPlaylist, show::FullEpisode, show::SimplifiedShow, track::FullTrack, Id,
  },
};
  ...
  pub fn from_type(t: FormatType) -> Vec<Self> {
    match t {
      FormatType::Album(a) => {
        ...
        if let Some(uri) = a.id.map(|id| id.uri()) {
      ...
      FormatType::Track(t) => {
        ...
        vec![
          ...
          Self::Track(t.name),
          Self::Uri(t.id.map(|id| id.uri()).unwrap_or_default()),
        ]
      }
      FormatType::Show(r) => vec![Self::Show(r.name), Self::Uri(r.id.uri())],
  ...
  pub fn inner(&self, conf: UserConfig) -> String {
    match self {
      ...
      Self::Position((curr, duration)) => {
        crate::tui::layout::display_track_progress(*curr as u128, *duration)
      }
      Self::Flags((r, s, l)) => {
        let like = if *l {
