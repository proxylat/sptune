...
impl PlaylistCache {
  ...
  /// Insert a placeholder item for the added uri and record the new
  /// snapshot id. Enough for the ✓ marker / playlist_uri_map to update
  /// without a full refetch; the full track metadata arrives on next open.
  pub fn add_item(&mut self, playlist_id: &str, uri: &str, snapshot: String) {
    if !CACHE_ENABLED.load(Ordering::Relaxed) {
      return;
    }
    self.ensure_loaded();
    ...
    if entry
      .items
      .iter()
      .any(|item| playlist_item_uri(item).as_deref() == Some(uri))
    {
      self.save();
      return;
    }
    entry.total = entry.total.saturating_add(1);
    // Minimal PlaylistItem with only the uri (name “Track” is enough for
    // playlist_uri_map which only keys on uri; the track list will refetch
    // the real metadata on next open).
    if let Ok(item) = serde_json::from_value::<PlaylistItem>(serde_json::json!({
      "added_at": null,
      "added_by": null,
      "is_local": false,
      "item": {
        ...
        "external_urls": {},
        "href": null,
        "id": uri.rsplit(':').next().unwrap_or("track"),
        "is_local": false,
        "is_playable": true,
        "name": "Track",
        "popularity": 0,
